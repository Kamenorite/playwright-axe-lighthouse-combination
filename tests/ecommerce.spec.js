...
test.describe('E-commerce Website Performance and Accessibility Tests', () => {
  ...
  test.afterAll(async () => {
    ...
    console.log(
      `Generating consolidated report with ${results.length} results from state`,
    );
    generateConsolidatedReport();
  });
  ...
  for (const page of TEST_PAGES) {
    test(`should test ${page.name}`, async ({
      page: pageContext,
    }, testInfo) => {
  ...
  test('should test product detail page', async ({ page }, testInfo) => {
    ...
    await page
      .waitForSelector('.product-deatil', { timeout: 10000 })
      .catch(() => {
  ...
  test('should test cart page', async ({ page }, testInfo) => {
    ...
    await page
      .waitForSelector('.table-responsive', { timeout: 10000 })
      .catch(() => {
